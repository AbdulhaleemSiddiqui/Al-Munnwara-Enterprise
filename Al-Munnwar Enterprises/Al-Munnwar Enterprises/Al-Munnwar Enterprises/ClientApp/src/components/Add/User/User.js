...
export class User extends Component {
    ...
    SaveUser = async () => {
        ...
        try {
            if (!this.validation()) {
                const result = await fetch("api/SaveUser", {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': 'application/json'
    ...
    validation = () => {
        ...
        if (this.state.name === "" || this.state.name === undefined) {
        ...
        else if (this.state.phoneNbr === "" || this.state.phoneNbr === undefined) {
            this.setState({
                errorMessage: "*User Phone Number is required.",
                isError: true
            });
            error = true;
        }
            ...
            this.setState({
                errorMessage: "*User Passwordf is required.",
                isError: true
            });
            error = true;
        }
            ...
            this.setState({
                errorMessage: "*Email formate is incorrect.",
                isError: true
            });
            error = true;
        }
        else if (this.state.phoneNbr.length < 10) {

            this.setState({
                errorMessage: "*Phone Number length  is incorrect.",
                isError: true
            });
            error = true;
        }
            ...
            this.setState({
                errorMessage: "*Password Length is soo short.",
                isError: true
            });
            error = true;
        }
    ...
    handleDelete(id) {
        this.setState({}, () => { this.DeleteUser(id) });
    }


    toggle = () => {
        this.setState({ modalCreateNewUser: true })
    }
    ...
    renderTableData() {
        return this.state.UserList.map((User, index) => {
            const { id, name,email,phoneNbr,password} = User
            return (
                <tr>
                    <td class="col-2">{name}</td>
                    <td class="col-2">{email}</td>
                    <td class="col-2">{phoneNbr}</td>

                    <td class="col-2">
                        <button class="btn btn-primary" style={{ marginRight: '10px' }} to={this.props.myroute} title="Edit" onClick={() => { this.handleEdit(User) }} ><FontAwesomeIcon icon={faEdit} /></button>
        ...
        return (
            ...
            <div>
                ...
                <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                    <button onClick={this.handleShow} class="btn btn-primary me-md-2" type="button"> Create User</button>
                </div>
                <div>
                        ...
                        <div style={{ display: this.state.showFilters }}>
                            <form className="form-inline">
                                <div className="col-12 d-flex" style={{ marginBottom: "5px", paddingLeft: "0px" }}>
                                    <table>
                                        <tr>
                                            <td><label for="filterrecord" style={{ width: "inherit" }} class="p-2">User Name</label></td>
                                            <td>
                    ...
                    <table class='table table-striped table-bordered mb-0' aria-labelledby="tableLabel">
                        <thead id="thead">
                            <tr style={{ cursor: "pointer" }}>
                                <th class="col-2" style={{ display: this.state.NoRecordStyle }} onClick={() => this.sorting('name')}>User Name</th>
                                <th class="col-2" style={{ display: this.state.NoRecordStyle }} onClick={() => this.sorting('name')}>Email</th>
                                <th class="col-2" style={{ display: this.state.NoRecordStyle }} onClick={() => this.sorting('name')}>Phone Number</th>
                ...
                <Modal show={this.state.setShow} onHide={this.handleClose} className="Large-Modal">
                    ...
                    <Modal.Body>
                        ...
                        <div className="row">
                            ...
                            <div className="col-5" style={{ display: 'flex' }}><input  required value={this.state.name} style={{ flexGrow: 1, float: 'left' }} type="text" maxLength="50" className="form-control mr-2" name="name" onChange={this.handleChange} /> <span style={{ flexGrow: 1, float: 'left', color: 'red' }}>*</span></div>
                        ...
                        <div className="row">
                            ...
                            <div className="col-5" style={{ display: 'flex' }}><input required value={this.state.email} style={{ flexGrow: 1, float: 'left' }} type="text" maxLength="50" className="form-control mr-2" name="email" onChange={this.handleChange} /> <span style={{ flexGrow: 1, float: 'left', color: 'red' }}>*</span></div>
                            <div className="col-2"></div>
                        </div>
                        <br />
                        <div className="row">
                            ...
                            <div className="col-5" style={{ display: 'flex' }}><input required value={this.state.phoneNbr} style={{ flexGrow: 1, float: 'left' }} type="text" maxLength="50" className="form-control mr-2" name="phoneNbr" onChange={this.handleChange} /> <span style={{ flexGrow: 1, float: 'left', color: 'red' }}>*</span></div>
                            <div className="col-2"></div>
                        </div>
                        <br />
                        <div className="row">
                            ...
                            <div className="col-5" style={{ display: 'flex' }}><input  required value={this.state.password} style={{ flexGrow: 1, float: 'left' }} type="password" maxLength="50" className="form-control mr-2" name="password" onChange={this.handleChange} /> <span style={{ flexGrow: 1, float: 'left', color: 'red' }}>*</span></div>
                            <div className="col-2"></div>
                        </div>
                    </Modal.Body>
                    <Modal.Footer>
                ...
                <div class="d-flex-justify-content-between div margin adjust-15">
                    <div className="justify-content-start overflow-auto">
                        ...
                        {this.state.totalRecords > 0 ? <CustomPagination
                            ...
                            recordsHandler={(e) => { this.getUser(e) }}
